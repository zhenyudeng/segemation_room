...
protected:
    ...
    void FastMergeRoomPair(std::vector<Room>& rooms, const int target_index, const int room_to_merge_index, cv::Mat& segmented_map, const double map_resolution);
    ...
    void FastCreateVoronoiGraph(cv::Mat& map_for_voronoi_generation,std::vector<cv::Point>& v_voronoi_pts);
    ...
    void FastPruneVoronoiGraph(cv::Mat& voronoi_map,
    ...
    void FastMergeRooms(cv::Mat& map_to_merge_rooms, std::vector<Room>& rooms, double map_resolution_from_subscription, double max_area_for_merging, bool display_map);
